...
fun main() {
    ...
    inputArray.forEach {
        val min = it.substringBefore("-").toInt()
        val max = it.substringAfter("-").substringBefore(" ").toInt()
        val character = it.substringBefore(":").substringAfter(" ")
        val password = it.substringAfter(":").trim()
        
        //Part 1
        val charCount = password.count { it.toString() == character }
        if(charCount in min..max) result1++
        
        //Part 2
        if((password[min - 1].toString() == character) && (password[max - 1].toString() != character) ||
           (password[min - 1].toString() != character) && (password[max - 1].toString() == character)) 
        		result2++
    }
