...
fun main() {
    val questionsByGroup = input.split("\n\n")
    val mergePersons = questionsByGroup.map { it.replace("\n", " ").replace(" ", "") }

    //Part 1
    val sets = mergePersons.map { it.toSet() }
    val anyoneSum = sets.sumBy { it.size }
    println(anyoneSum)

    //Part 2
    val givePeopleSomeSpace = questionsByGroup.map { it.replace("\n", " ") }
    val everyoneSum = givePeopleSomeSpace.sumBy {
        val questionByPerson = it.split(" ")
        val commonQuestions = questionByPerson.fold(questionByPerson[0].toSet()) {
                acc, element -> acc.intersect(element.toSet())
        }
