val input = """eyr:2024 pid:662406624 hcl:#cfa07d byr:1947 iyr:2015 ecl:amb hgt:150cm
...
fun main() {
    ...
    val basicValidPassports = inputArray.filter { validatePassport(it) }
    
    val fullyValidPassports = basicValidPassports.filter {
        validateByr(it) &&
        validateIyr(it) &&
        validateEyr(it) &&
        validateHgt(it) &&
    ...
    println(basicValidPassports.count())
    println(fullyValidPassports.count())
}

fun validatePassport(data: String): Boolean {
...
fun validateByr(data: String): Boolean {
    val byr = data.substringAfter("byr:").substringBefore(" ")
    return byr.length == 4 && byr.toInt() >= 1920 && byr.toInt() <= 2002
}

fun validateIyr(data: String): Boolean {
    val iyr = data.substringAfter("iyr:").substringBefore(" ")
    return iyr.length == 4 && iyr.toInt() >= 2010 && iyr.toInt() <= 2020
}

fun validateEyr(data: String): Boolean {
    val eyr = data.substringAfter("eyr:").substringBefore(" ")
    return eyr.length == 4 && eyr.toInt() >= 2020 && eyr.toInt() <= 2030
}

fun validateHgt(data: String): Boolean {
    val hgt = data.substringAfter("hgt:").substringBefore(" ") 
    val hgtCmValue = if(hgt.contains("cm")) hgt.substringBefore("cm").toInt() else 0
    val hgtInValue = if(hgt.contains("in")) hgt.substringBefore("in").toInt() else 0
    return ((hgtCmValue >= 150 && hgtCmValue <= 193) || (hgtInValue >= 59 && hgtInValue <= 76))
}

fun validateHcl(data: String): Boolean {
    val hcl = data.substringAfter("hcl:").substringBefore(" ") 
...
fun validateEcl(data: String): Boolean {
    val ecl = data.substringAfter("ecl:").substringBefore(" ") 
    return ecl.length == 3 &&
    ecl == "amb" || 
    ecl == "blu" ||
    ecl == "brn" ||
...
fun validatePid(data: String): Boolean {
    val pid = data.substringAfter("pid:").substringBefore(" ") 
    if(pid.length != 9) return false
    return if(pid.toIntOrNull() != null) true else false
}
